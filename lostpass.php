...
if (isset($_POST['email'])) {
    ...
    if ($stmt = $conn->prepare($sql_code)) {
        ...
        try {
            ...
            $mail->Body    = 'Clique no link para redefinir sua senha: <a href="http://ateliealinenacur.shop/codigo.php?token=' . $token . '">Redefinir Senha</a><br>Seu código de verificação é: ' . $verification_code;
            $mail->AltBody = 'Clique no link para redefinir sua senha: http://ateliealinenacur.shop/codigo.php?token=' . $token . ' Seu código de verificação é: ' . $verification_code;
