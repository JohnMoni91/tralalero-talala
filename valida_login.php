...
if ($stmt = $conn->prepare("SELECT id, senha, is_admin FROM login_clientes WHERE email = ?")) {
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->bind_result($id_cadastrado, $senha_cadastrada, $is_admin);
    $stmt->fetch();
    ...
    if (password_verify($senha, $senha_cadastrada)) {
        ...
        $_SESSION['user_id'] = $id_cadastrado; // Armazena o ID do usuário na sessão
